...
const CreateExperiencePage = ({ onReturn }) => {
  ...
  const generateContent = async () => {
    ...
    try {
      ...
      if (selectedFormat === 'image-only' || selectedFormat === 'text-image' || selectedFormat === 'image-video') {
        try {
          ...
          // Start countdown for video generation if needed
          if (selectedFormat === 'image-video') {
          ...
          if (imageResponse.ok) {
            ...
            // Generate video if image-video format is selected
            if (selectedFormat === 'image-video') {
              try {
                ...
                reader.onloadend = async () => {
                  ...
                  const videoResponse = await fetch('https://api.segmind.com/v1/ltx-2-19b-i2v', {
                    ...
                    body: JSON.stringify({
                      prompt: refinedPrompt + moodPromptAddition,
                      image: base64Image,
                      width: 1024,
                      height: 1024,
                      num_frames: 25,
                  ...
                  if (videoResponse.ok) {
                    ...
                    // Update generated content with video
  ...
  const handleGenerateVideo = (image, originalPrompt) => {
    // Open video generation in new tab
    const videoGenerationUrl = `${window.location.origin}${window.location.pathname}?video-generation=true`;
    const newTab = window.open(videoGenerationUrl, '_blank');
    
    // Pass data to new tab via localStorage (temporary storage)
    const videoData = {
      imageUrl: image.url,
      originalPrompt: originalPrompt,
      timestamp: Date.now()
    };
    localStorage.setItem('video-generation-data', JSON.stringify(videoData));
  };
    ...
    <div className="create-experience-page">
      <div className="create-experience-container">
        ...
        <div className="creation-panel">
                ...
                >
                  {isGenerating ? 
                    (selectedFormat === 'image-only' ? '🎨 Painting your vision...' :
                     ...
                     selectedFormat === 'image-video' ? '🎬 Creating image and video...' :
          ...
          {(isGenerating || isVideoGenerating) && selectedFormat === 'image-video' && (
              ...
              <div className={`generated-content ${generatedContent.text && generatedContent.image ? 'side-by-side' : ''}`}>
                ...
                {generatedContent.video && (
                  <div className="generated-video">
                    <h4>🎬 Video:</h4>
                    <video src={generatedContent.video.url} controls />
                  </div>
                )}
              </div>
              <div className="content-actions">
                {generatedContent.image && (selectedFormat === 'image-only' || selectedFormat === 'text-image') && (
                  <button 
                    onClick={() => handleGenerateVideo(generatedContent.image, refinedPrompt)}
